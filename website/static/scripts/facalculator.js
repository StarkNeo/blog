...
const indicesDB = new Object();

indices.forEach(element => {
    if (indicesDB[element[2]] === undefined) {
        indicesDB[element[2]] = { [monthNames[element[1] - 1]]: element[3] }
    } else {
        indicesDB[element[2]][monthNames[[element[1]] - 1]] = element[3];
    }
...
console.log(indicesDB);
//INDICES 2023 EN ADELANTE ACTUALIZACION MENSUAL
nuevosIndices={
    
}

//OBJETO QUE ALMACENARA LA INFORMACION DEL ACTIVO
const ASSET = {
    adq_date: {},
    sell_date: {},
    opt_date: {},
...
//getDate toma la fecha capturada del input de fecha de adquisicion y el de fecha de venta y la transforma en numeros
//2024-02-05 es la fecha que se envia al script desde el DOM, 
//con substring tomo los caracteres de la posicion 5 al 7 para el mes y 0 al 4 para el ejercicio y los transformo en numeros para buscar en la base de datos
...
const getDate = async (id, value) => {
    ...
    if (id === 'adqDate') {
        ...
        let indice = await getInpc(monthNames[date.getMonth()], year)
        ASSET.inpcAdq = Number(indice.toFixed(4));
...
//ESTA FUNCION BUSCA EL INDICE EN LA BASE DE DATOS POSTGRES
//PARA ESTE DEMO USO UN ARREGLO COMO BASE DATOS
async function getInpc(month, year) {

    try {
        return indicesDB[year][month];
    } catch (error) {
        console.log(error);
    }
};
...
//ESTA FUNCION SE EJECUTA CUANDO SE OPTA POR CAMBIAR LA FORMA DE CALCULO PREDETERMINADA
const getIndex = async () => {
    ASSET.option === false ? getLastPeriod(ASSET.adq_date, ASSET.sell_date) : getLastPeriod(ASSET.adq_date, ASSET.opt_date);
    try {
        let index = await getInpc(monthNames[ASSET.last_period.month - 1], ASSET.last_period.year)
        //Almacena el indice en el objeto
        ASSET.last_period.index = Number(index.toFixed(4));
...
function getLastPeriod(dateRef1, dateRef2) {
    ...
    if (isSame) {
        firstHalfMonth = dateRef1.month + Math.floor((dateRef2.month - dateRef1.month) / 2)
        
    } else {
        let diference = Math.floor((((dateRef2.month - 1) + 1) / 2) - 1)
        if (diference < 1) {
            firstHalfMonth = 1;
        } else {
            firstHalfMonth = 1 + diference
        }
